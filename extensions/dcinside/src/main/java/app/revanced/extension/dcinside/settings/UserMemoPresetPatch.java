...
public final class UserMemoPresetPatch {
    private static final int MAX_MEMO_LENGTH = 15;
    private static final String GLOBAL_GALLERY_ID = "";
    private static final String USER_MEMO_PRESET_RESOURCE_NAME = "user_memo_preset_1";

    private static volatile String cachedPreset;

    private UserMemoPresetPatch() {
    }
    ...
    public static void registerUserMemoPreset(Context context) {
        ...
        List<MemoEntry> entries = parsePreset(getPreset(context));
        if (entries.isEmpty()) {
    ...
    private static String getPreset(Context context) {
        String preset = cachedPreset;
        if (preset != null) {
            return preset;
        }

        preset = readPreset(context);
        cachedPreset = preset;
        return preset;
    }

    private static String readPreset(Context context) {
        Resources resources = context.getResources();
        int resourceId = resources.getIdentifier(
                USER_MEMO_PRESET_RESOURCE_NAME,
                "raw",
